...
class DisplayPacketDecoder:

    def get_display_frame_buffer(self, data_packet):

        decoded_frames = self._decode_packet(data_packet)
        return DisplayBuffer(decoded_frames)

    def _decode_packet(self, data_packet):
            ...
            for component in range(component_count):
                ...
                match component_type:
                    ...
                    case FrameComponent.TEXT:
                        components.append(self._decode_text_packet_slice(component_data_slice, draw_colour))
                    case FrameComponent.INSTRUCTION:
                        components.append(self._decode_instruction_packet_slice(component_data_slice, draw_colour))
    ...
    def _decode_text_packet_slice(self, packet_slice, draw_colour):
    ...
    def _decode_instruction_packet_slice(self, packet_slice, draw_colour):
        ...
        match instruction_type:
            ...
            case InstructionComponent.POLYGON_EMPTY:
                x = int.from_bytes(packet_slice[1:3], 'big')
                y = int.from_bytes(packet_slice[3:5], 'big')
                coords = self._decode_coords(packet_slice[5:])
    ...
    def _decode_coords(self, coords_slice):
        coords = array('h')
